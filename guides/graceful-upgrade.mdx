---
title: "Graceful Zero-Downtime Binary Upgrade"
sidebarTitle: "Graceful Upgrade"
description: "Upgrade a running Pingora server binary with zero dropped connections using the SIGQUIT signal and --upgrade flag."
---
...
For a quick local upgrade (e.g., during development), you can run both steps together:

```bash
pkill -SIGQUIT load_balancer && RUST_LOG=INFO cargo run -- -c conf.yaml -d -u
```

This sends SIGQUIT to the old process and immediately launches the new one with `--upgrade`, relying on the upgrade socket to synchronize the handover.
